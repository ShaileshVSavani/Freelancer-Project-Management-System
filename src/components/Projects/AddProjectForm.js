...
import { toast, ToastContainer } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";
import { ProjectPaymentsContext } from "../../context/ProjectPaymentsContext"; // Import context

const AddProjectForm = ({ onSave = () => {}, onUpdateProject, existingProject }) => {
  const { projects, setProjects } = useContext(ProjectPaymentsContext); // Access context
  ...
  const handleSubmit = (e) => {
    ...
    if (existingProject) {
      ...
      // Save the new projects list to localStorage
...
// Default props are handled by default function parameters
AddProjectForm.defaultProps = {
  onSave: () => {},
};
